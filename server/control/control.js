...
e.Editar= async (req, res) =>{
  ...
  const a={status: req.body.status }
   await empleado.updateOne(/*req.params.id*/ {"_id": req.params.id},{$set:a ,/*id,*/  $addToSet: emp },{upsert:true},(err, doc)=>{// actualiza cualquier registro
    if(err){
      console.log('not ok');
    }else{
      e.Crear();
